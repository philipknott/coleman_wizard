...
function layoutFrame(frame, org) {
	...
	//console.log("in layoutFrame; topLeft, org: ",topLeft,org);
	if (org.y > topLeft.y) //don't try to draw null box
...
function layoutSimpleSlot(part, org) {
	...
	putBox(part.question, part.value, org.x, org.y, part, 0);
	org.x++;
	org.y++;
...
function layoutCheckBoxes(part, org) {
	...
	for (var i = 0; i < part.options.length; i++) {
		if (thisCheckOptionIsInValue(part, i))
			boxLabel = makeCheckboxLabelChecked();
		else boxLabel = makeCheckboxLabelUnchecked();
...
function thisCheckOptionIsInValue(part, i) {
	for (var j = 0; j < part.value.length; j++)
		if (part.value[j] == part.options[i].choice)
			return true;
	return false;
}
...
function layoutAlternativesFrame(frame, org) {
	...
	for (var i = 0; i < frame.choices.length; i++) {
		if (thisRadioChoiceIsOn(frame, i))
			boxLabel = makeRadioLabelOn(i, frame.choices.length);
		else boxLabel = makeRadioLabelOff(i, frame.choices.length);
		//putBox(frame.choices[i].question,org.x,org.y);
		putBox(frame.choices[i].question, boxLabel, org.x, org.y, frame, i); //slot set to frame
		org.y++;
...
function thisRadioChoiceIsOn(frame, i) {
	console.log("checking radio choice; frame, i: ", frame, i);
	var answer;
	if (frame.choice == "")
		answer = false;
	else {
		answer = (frame.choice.name == frame.choices[i].subframe.name);
		console.log("choice name, name in choice i ", frame.choice.name, frame.choices[i].subframe.name);
	}
	console.log("answer is: ", answer);
	return answer;
}
function makeRadioLabelOff(i, n) {
	if (i == 0)
		return "|( )";
	if (i == (n - 1))
		return "( )|";
...
function drawRect(color, upperLeft, lowerRight) {
	//console.log("in drawRect; color,upperLeft,lowerRight: ",color,upperLeft,lowerRight);
...
function clickedOnRect(e) {
	...
	else if (slots[i][j].type == "more")
		processMorePar(i, j, slots[i][j]);
	else console.log("bad slot type");
}
function processMorePar(i, j, slot) {
	console.log("processMore; i, j: ", i, j);
	//remove existing more tag
	removeClickableRect(i, j);
	//make a bound copy of action frame
	//var newFrame=clone(slot.subframe); //this does just action
	var newFrame = clone(topLevel[0]); //note: change these indices for general case
	topLevel[topLevel.length] = newFrame;
	//lay it out
	layoutFrame(newFrame, { x: i, y: j }); //is this the right place? yes
	//add the clone to the data structure, too, so code gets generated
	//note this will be at the end of Top
	//topLevel[topLevel.length]=newFrame;
	console.log("after processMorePar, topLevel is now: ", topLevel);
}
...
function radioIsOn(i, j) {
	console.log("in radioIsOn i j vals[][j]:", i, j, vals[i][j]);
...
function turnRadioOffLeft(i, j) {
	console.log("turnRadioOffLeft i j vals[i][j]: ", i, j, vals[i][j]);
	if (isLeftEnd(i, j))
		return;
...
function setValueRadio(i, j) {
	var slot = slots[i][j];
	slot.choice = slot.choices[index[i][j]].subframe; //do NOT want clone here!
	console.log("choice:", slot.choice);
	displayColor(1, topLevel);
}
/*
...
function removeClickableRect(i, j) {
	var id = i + ":" + j;
	console.log("removing ", i, j);
	//const svg = document.getElementById("firstGroup");
	//document.getElementById(id).remove();
	document.getElementById("label" + i + ":" + j).remove();
	//writeValue(i,j,"dummy");
}
